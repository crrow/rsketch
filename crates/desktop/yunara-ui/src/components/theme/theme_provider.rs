...
///
/// This module implements the GPUI Global pattern to provide theme configuration
/// to all UI components. Components access the current theme via:
/// ```ignore
...
impl ThemeProvider {
    ...
    pub fn init(cx: &mut App) {
        cx.set_global(Self::new(ThemeConfig::default()));
    }

    /// Initializes the theme provider with a specific theme configuration.
    pub fn init_with_theme(cx: &mut App, theme: ThemeConfig) {
        cx.set_global(Self::new(theme));
    }
}
...
impl ThemeExt for App {
    fn theme(&self) -> &ThemeConfig {
        self.global::<ThemeProvider>().theme()
    }
}
